...
def create_user(name, email):
    response = get_user_id(None, name, email)
...
def run_general_setup(user_name, email, wallet_name, wallet_password):
    user_id = create_user(user_name, email)
...
class ViewsTestCase(TestCase):
    ...
    def test_get_user_id_new_user_returns_empty_wallet(self):
        response = get_user_id(None, self.user, self.email)
        new_user = get_user(self.user, self.email)
    ...
    def test_get_user_id_old_user_returns_existing_wallet(self):
        _ = get_user_id(None, self.user, self.email)
        new_user = get_user(self.user, self.email)

        existing_wallet = new_user.walletdetails_set.all().first()
        existing_wallet.wallet_name = self.wallet_name
    ...
    def test_create_wallet_attempts_to_create_wallet(self):
        user_id = create_user(self.user, self.email)
    ...
    def test_open_wallet_attempts_to_open_wallet(self):
        ...
        user_id = run_general_setup(new_user, new_email, new_wallet_name, new_wallet_password)
